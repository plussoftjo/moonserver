            ...
            <table class="table">
              ...
              <tr v-for="(item, index) in data" :key="index">
                <slot :row="item">
                  ...
                  <td>
                      <span style="color:green;font-weight:700;cursor:pointer;" @click="$router.push({name:'QuizShow',params:{id:item.id}})">متابعة العملية</span>
                  </td>
                </slot>
              </tr>
              </tbody>
...
<script>
  ...
  export default {
    ...
    methods: {
      install() {
        const vm = this;
        axios.get('admin/process/quiz').then(response => {
