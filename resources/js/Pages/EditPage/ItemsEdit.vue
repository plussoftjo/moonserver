                   ...
                   <div class="container">
                       ...
                       <div style="height:20px;"></div>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label for="categorys">الفئة</label>
                                    <select class="form-control" id="categorys" v-model="form.shop_category_id">
                                        <option v-for="(catg,index) in categorys" :key="index" :value="catg.id">{{catg.name_ar}}</option>
                                    </select>
                                </div>
                            </div>
                            <div class="col-md-6">
                        ...
                        <div class="row" v-if="error">
                            <div class="col-md-12">
                                <div class="error">
                                    يوجد خطا في الحقول الرجاء اكمال الحقول 
                                </div>
                            </div>
                        </div>
                        <div class="row">
  ...
  export default {
    ...
    methods: {
        ...
        updateImage() {
            ...
            vm.show_loader = true;
            vm.error = false;
            axios.post('admin/shop_items/updateImage/' + vm.$route.params.id,{image:vm.form.image}).then(response => {
        ...
        update() {
            ...
            vm.show_loader = true;
            vm.error = false;
            axios.post('admin/shop_items/update/' + vm.$route.params.id,vm.form).then(response => {
                 vm.show_loader = false;
                vm.error = false;
                confirm('تم تعديل المعلومات بنجاح') && 
    ...
    mounted() {
        const vm = this;
        axios.get('admin/shop_category/index').then(response => {
        ...
        function install_form() {
            axios.get('admin/shop_items/show/' + vm.$route.params.id).then(response => {
