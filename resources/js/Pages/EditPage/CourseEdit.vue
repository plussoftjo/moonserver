                       ...
                       <div style="height:20px;"></div>
                        ...
                        <div class="rw">
                            <div class="col-md-12" v-if="error">
                                <div class="error">
                                    يوجد خطا في الحقول الرجاء التاكد من جميع الحقول 
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <button class="btn btn-success btn-block" @click="update">حفظ المعلومات</button>
                            </div>
                            <div class="col-md-3"></div>
                        </div>
                   </div>
...
<script>
  ...
  export default {
    ...
    methods: {
        ...
        updateImage() {
            const vm = this;
            axios.post('admin/course/updateImage/' + vm.$route.params.id , {image:vm.form.image}).then(response => {
                confirm('تم التحديث بنجاح') && 
                location.reload();
            }).catch(err => {
                console.log(err)
            });  
        ...
        store() {
            const vm = this;
            axios.post('admin/course/store',vm.form).then(response => {
        ...
        update() {
            ...
            vm.show_loader = true;
            vm.error = false;
            axios.post('admin/course/update/' + vm.$route.params.id,vm.form).then(response => {
                vm.show_loader = false;
                vm.error = false;
                confirm('تم التحديث بنجاح') && 
                vm.$router.push({name:'معلومات الدورة',params:{id:vm.$route.params.id}});
            }).catch(err => {
                console.log(err)
                vm.error = true;
                vm.show_loader= false;
